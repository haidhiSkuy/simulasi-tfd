...
def solution_C5():
    ...
    model.compile(optimizer='sgd', loss=tf.keras.losses.MeanAbsoluteError(), metrics=['mae'])
    
    es = tf.keras.callbacks.EarlyStopping(monitor='val_mae',patience=15,mode='min')
    model.fit(train_set, validation_data=valid_set, epochs=500, callbacks=[es])
    return model
